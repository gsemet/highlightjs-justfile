...
const fs = require('fs');
const hljsDefineJustfile = require('../src/justfile');
hljs.registerLanguage('justfile', hljsDefineJustfile);

const readdir = promisify(fs.readdir);
const readFile = promisify(fs.readFile);

describe('justfile syntax highlighting', () =>
{
  async function itShouldPerformSyntaxHighlighting()
  {
    ...
    scenarios.forEach(scenario => {
      it(`should perform syntax highlighting on ${scenario}`, async () => {
        ...
        const result = hljs.highlight('justfile', code);
  ...
  // The following test is ignored because the language detected is makefile.
  // Since the justfile syntax can be confused with Makefile,
  // I don't think we should use "highlightAuto" (as it will produce unexpected results)
  // Please note that Highlight.js can also return css or lisp depending on the code used.
  xit('should detect justfile language', async () =>
  {
    var code = await readFile(path.join(__dirname, 'detect', 'default.txt'), 'utf-8');
    var actual = hljs.highlightAuto(code).language;
    actual.should.eql('justfile');
  });
});
